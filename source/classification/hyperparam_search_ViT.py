...
def generate_list_for_y_test(available_ids_list, available_ids_dict, batch_size):  
    y_test_class_names = []
    num_batch = len(available_ids_list) // batch_size
    limit_point = num_batch * batch_size
    print(f"The upper point is: {limit_point}")
    for ids in available_ids_list:
        cell_specimen_id = int(ids)
        key = "%d" %cell_specimen_id
        cell_dir_PatchSeqGouwens = os.path.join(output_cwt_dir_PatchSeqGouwens,"cell_%d.npy" %cell_specimen_id)
        ...
        col_size = 4800
        if cell_cwt.shape == (180,col_size, 2):
            if len(y_test_class_names) < limit_point:
...
def BayesSearch():
    ...
    # Arrange the size of y_test based on y_pred
    num_batch = len(y_test_class_names) // batch_size_best
    upper_limit = num_batch * batch_size_best
    y_test_class_names = y_test_class_names[:upper_limit]
...
class MyEstimator:
    ...
    def score(self, X_test_inner, y_train_inner, **params):
        ...
        y_test_inner_class_names = generate_list_for_y_test(X_test_inner, available_ids_dict, batch_size)
    ...
    def _load_model(self):
        ...
        #Check which pre-trained model is wanted to be used
        if self.model_no == 1:
            #Load ViT Base 16 model (vit_b_16) model
            model = models.vit_b_16(pretrained=True)
            num_features = model.num_classes           
            
            if dropout_rate == 0:
                model.num_classes = nn.Linear(num_features, 4)
            else:
                model.num_classes = nn.Sequential(
                    nn.Dropout(p=dropout_rate),  # Dropout layer with 50% probability
                    nn.Linear(num_features, 4)  # Fully connected layer with 4 output features
                )
            size_model = (224, 224)
        elif self.model_no == 2:
            #Load ViT Base 32 model (vit_b_32) model
            ...
            #Load ViT Large 16 model (vit_l_16) model
            ...
            #Load ViT Large 32 model (vit_l_32) model
            ...
            #Load ViT Huge 14 model (vit_h_14) model
